...
function setMutationObserver() {
  const targetNode1 = document.querySelector(".columns-area__panels")
    ? document.querySelector(".columns-area__panels")
    : document.querySelector(".columns-area");
  const targetNode2 = document.querySelector(".modal-root");
  const config = { attributes: false, childList: true, subtree: true };
  function callback(mutationsList, observer) {
    document
      .querySelectorAll(".columns-area .media-gallery__item-thumbnail img")
      .forEach((item) => changeSrc(item));
    document
      .querySelectorAll(".columns-area .video-player video")
      .forEach((item) => changeSrc(item));
    document
      .querySelectorAll(".columns-area .media-gallery__gifv video")
      .forEach((item) => changeSrc(item));
    document
      .querySelectorAll(".modal-root .image-loader img")
      .forEach((item) => changeSrc(item));
  }
